...
class Api
{
    ...
    {
        if (!$auth) {
            // Todo: this has to be done better. Either through config or falling back to defaults or something.
            $staticDir = ROOTPATH . "/static";
            $proxy = new \NI\Api\Proxy($this->api_dir, \NI::$token, $this->api_key, $this->auth_url, $staticDir);
        ...
        $jsonCall = false;
        if (!empty($_SERVER['CONTENT_TYPE']) && substr($_SERVER['CONTENT_TYPE'], 0, strlen($jsonType)) === $jsonType) {
            $jsonCall = true;
        }
